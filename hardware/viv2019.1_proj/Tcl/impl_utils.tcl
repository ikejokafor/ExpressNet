...
proc get_ooc_results { implementations } {
   ...
   foreach ooc $implementations {
      ...
      set readCheckpoint [get_ooc_attribute $ooc implCheckpoint]
      set preservation   [get_ooc_attribute $ooc preservation]
      set hd.isolated    [get_ooc_attribute $ooc hd.isolated]

      if {![file exists $readCheckpoint]} {
      set errMsg "ERROR: Specified OOC Checkpoint $readCheckpoint does not exist"
      error $errMsg
      }
      ...
      if {[string match $preservation "logical"] || [string match $preservation "placement"] || [string match $preservation "routing"]} {
         ...
         set errMsg "ERROR: Unknown value \"$preservation\" specified for lock_design for cell $hierInst."
         error $errMsg
      }
   }
}
...
proc generate_pr_bitstreams { configs } {
   ...
   foreach config $configs {
      set top         [get_config_attribute $config top]
      set settings    [get_config_attribute $config settings]
      set bitstream   [get_config_attribute $config bitstream]
      set bitOptions  [get_config_attribute $config bitstream_options]
      set bitSettings [get_config_attribute $config bitstream_settings]
      if {$bitstream} {
         ...
         foreach setting $settings {
            ...
            if {[string match $cell $top]} {
               ...
               if {[file exists $configFile]} {
                  ...
                  command "write_bitstream $bitOptions $bitDir/$config" "$bitDir/$config.log"
               } else {
               ...
               if {[string match $state "implement"]} {
                  ...
                  command "write_bitstream $bitOptions $bitDir/blanking" "$bitDir/blanking.log"
               }
            }
         }
         command "close_project" "$bitDir/temp.log"
      } else {
         puts "\tSkipping write_bistream for Configuration $config with attribute bistream set to $bitstream"
      }
   }
}
...
proc verify_configs { configs } {
   ...
   set initialConfigTop [get_config_attribute $initialConfig top]
   ...
   for {set i 1} {$i < $numConfigs} {incr i} {
      ...
      if {$verify} {
         ...
         set configTop [get_config_attribute $config top]
...
proc add_xdc { xdc } {
   ...
   foreach file $files {
      if {[file exists $file]} {
         ...
         set errMsg "ERROR: Could not find specified XDC: $file" 
         error $errMsg 
      }
   }
}
   ...
   foreach file $files {
      if {[string length $file] > 0} { 
         if {[file exists $file]} {
            ...
            set errMsg "ERROR: Could not find specified IP netlist: $file" 
            error $errMsg
         }
      }
   }
...
# Critical Warnings. These will be errors in write_bistream. 
...
proc check_drc { module {ruleDeck all} {quiet 0} } {
   ...
   if {[llength $Errors]} {
      if {!$quiet} {
         puts "\tERROR: DRC found Errors ($Errors)."
      } else {
         puts "\tCritical Warning: DRC found Errors ($Errors)."
      }
   ...
   if {[llength $CritWarnings]} {
      if {!$quiet} {
         puts "ERROR: DRC found Critical Warnings ($CritWarnings)."
      } else {
         puts "\tCritical Warning: DRC found Critical Warnings ($CritWarnings)."
      }

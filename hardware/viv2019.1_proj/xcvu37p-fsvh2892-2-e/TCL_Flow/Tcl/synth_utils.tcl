   ...
   if {[file exists $prj]} {
      ...
      foreach line $prj_lines {
         ...
         if {[llength $line] > 0 && ![string match -nocase "#*" $line]} {
            if {[llength $line]!=3} {
               set errMsg "ERROR: Line $line_count is invalid format. Should be:\n\t<file_type> <library> <file>"
               error $errMsg
            }
            ...
            if {![string match -nocase $type "dcp"]     && \
               ...
               set errMsg "ERROR: File type $type is not a supported value.\n"
      ...
      if {[info exists error]} {
         set errMsg "ERROR: Files not found. Check messages for more details.\n"
         error $errMsg
      }
   } else {
      set errMsg "ERROR: Could not find PRJ file $prj"
      error $errMsg
   }
}
   ...
   foreach ip $ips {
      if {[string length ip] > 0} { 
         if {[file exists $ip]} {
            ...
            set errMsg "ERROR: Could not find specified IP file: $ip" 
            error $errMsg
         }
      }
   }
   ...
   foreach file $files {
      if {[string length file] > 0} { 
         if {[file exists $file]} {
            ...
            if {[get_property NEEDS_REFRESH [get_files $file]]} {
               command "generate_target all \[get_files $file]" "$resultDir/${bdName}_generate.log"
            }
         } else {
            set errMsg "ERROR: Could not find specified BD file: $file" 
            error $errMsg
         }
      }
   }
